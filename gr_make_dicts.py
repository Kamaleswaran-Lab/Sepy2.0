...
def sirs_summary(csn, instance):
    sirs_scores = instance.encounter_dict['sirs_scores'].reset_index().rename(columns = {'index':'time_stamp'})
...
def enc_summary(csn, instance):
    enc_summary_dict = {**instance.flags, **instance.static_features, **instance.event_times}
...
def comorbidity_summary(csn, instance):
    #ahrq_ICD9_dict[csn]=instance.ahrq_ICD9_PerCSN.icd_count
    #elix_ICD9_dict[csn]=instance.elix_ICD9_PerCSN.icd_count
    #quan_deyo_ICD9_dict[csn]=instance.quan_deyo_ICD9_PerCSN.icd_count
    #quan_elix_ICD9_dict[csn]=instance.quan_elix_ICD9_PerCSN.icd_count
    #ccs_ICD9_dict[csn]=instance.ccs_ICD9_PerCSN.icd_count
    ...
    #print(f'The csn for comorbids is: {csn}')
...
if __name__ == '__main__':
    ...
    csn_df.csn = csn_df.csn.astype('int')
    ...
    pickle_name = pickle_path / ('all_gr_y' + str(bash_year) + '.pickle')
    ...
    try:
        ...
        Path.mkdir(output_path / 'gr_sepsis_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'sofa_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'sep3_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'sirs_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'sep2_summary', exist_ok = True)       
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'encounter_summary', exist_ok = True)       
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'error_summary', exist_ok = True)
        
        #ICD9 co-morbid
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'ahrq_ICD9_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'elix_ICD9_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'quan_deyo_ICD9_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'quan_elix_ICD9_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'ccs_ICD9_summary', exist_ok = True)
        #ICD10 co-morbid
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'ahrq_ICD10_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'elix_ICD10_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'quan_deyo_ICD10_summary', exist_ok = True)
        Path.mkdir(output_path / 'gr_sepsis_summary' / 'quan_elix_ICD10_summary', exist_ok = True)
        #Path.mkdir(output_path / 'gr_sepsis_summary' / 'ccs_ICD10_summary', exist_ok = True)
        
        # write general files
        unique_file_id = "%s_%s"%(processor_assignment,bash_year)
        pd.concat(appended_enc_summaries).to_csv(output_path   / 'gr_sepsis_summary' / 'encounter_summary' /  ('encounters_summary_'+ unique_file_id +'.csv')  , index = True)      
        pd.DataFrame(error_list, columns = ['csn','error']).to_csv(output_path / 'gr_sepsis_summary' / 'error_summary' / ('error_list_'+ unique_file_id +'.csv')  , index = False)

        # write sepsis files 
        pd.concat(appended_sofa_scores).to_csv(output_path / 'gr_sepsis_summary' / 'sofa_summary' / ('sofa_summary_'+ unique_file_id +'.csv'), index = False)
        pd.concat(appended_sep3_time).to_csv(output_path / 'gr_sepsis_summary' / 'sep3_summary' / ('sepsis3_summary_'+ unique_file_id +'.csv'), index = False)
        pd.concat(appended_sirs_scores).to_csv(output_path   / 'gr_sepsis_summary' / 'sirs_summary' /  ('sirs_summary_'+ unique_file_id +'.csv'), index = False)
        pd.concat(appended_sep2_time).to_csv(output_path / 'gr_sepsis_summary' / 'sep2_summary' / ('sepsis2_summary_'+ unique_file_id +'.csv'), index = False)
 
        #write comorbidity files
        #ICD9
        #pd.DataFrame.from_dict(ahrq_ICD9_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'ahrq_ICD9_summary' / ('ahrq_ICD9_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(elix_ICD9_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'elix_ICD9_summary' / ('elix_ICD9_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(quan_deyo_ICD9_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'quan_deyo_ICD9_summary' / ('quan_deyo_ICD9_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(quan_elix_ICD9_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'quan_elix_ICD9_summary' / ('quan_elix_ICD9_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(ccs_ICD9_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'ccs_ICD9_summary' / ('ccs_ICD9_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #ICD10
        #pd.DataFrame.from_dict(ahrq_ICD10_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'ahrq_ICD10_summary' / ('ahrq_ICD10_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(elix_ICD10_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'elix_ICD10_summary' / ('elix_ICD10_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        pd.DataFrame.from_dict(quan_deyo_ICD10_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'quan_deyo_ICD10_summary' / ('quan_deyo_ICD10_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        pd.DataFrame.from_dict(quan_elix_ICD10_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'quan_elix_ICD10_summary' / ('quan_elix_ICD10_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #pd.DataFrame.from_dict(ccs_ICD10_dict).T.to_csv(output_path / 'gr_sepsis_summary' / 'ccs_ICD10_summary' / ('ccs_ICD10_summary_'+ unique_file_id +'.csv'), index = True, index_label='csn')
        #################################################
