...
Version: 0.2
Changes:
    - Removed from line 205: csn_df.csn = csn_df.csn.astype('int')
    - Add in line 237: bed_to_unit_mapping.drop(columns=["Unnamed: 0"], inplace=True)
...
DIALYSIS_INFO_CSN_FNAME = None  # Path('/labs/kamaleswaranlab/MODS/EliteDataHacks/sepy/new_sepy_mehak/PEACH_HD_CRRT(1).csv')


def process_csn(encounter_csn, pickle_save_path, bed_unit_mapping, thresholds, yearly_data_instnace):
    """
    ...
    Args:
        ...
        yearly_data_instance (sepyDICT): An instance of the `sepyDICT` class containing the yearly data for the patient
    Returns:
    ...
    # instantiate class for single encounter, no dialysis info is read
    encounter_instance = sd.sepyDICT(
        yearly_data_instnace, encounter_csn, bed_unit_mapping, thresholds, None
    )
...
def sirs_summary(encounter_csn, encounter_instance):
    ...
    Args:
        encounter_csn (int or str): The unique encounter ID (CSN) for the patient encounter.
        encounter_instance (sepyDICT): An instance of the encsepyDICTounter class, containing the encounter data.
    """
    sirs_scores = (
...
if __name__ == "__main__":
    ...
    try:
        ...
        for count, csn in enumerate(process_list, start=1):
            try:
                print(
                    f"MkDct- Processing patient csn: {csn}, {count} of {chunk_size} for year {bash_year}"
                )
                instance = process_csn(
                    csn,
                    pickle_write_path,
                    bed_to_unit_mapping,
                    bounds,
        ...
        Path.mkdir(
            output_path / "gr_sepsis_summary" / "encounter_summary", exist_ok=True
        )
        ...
        Path.mkdir(
            output_path / "gr_sepsis_summary" / "quan_deyo_ICD10_summary", exist_ok=True
        )
        Path.mkdir(
            output_path / "gr_sepsis_summary" / "quan_elix_ICD10_summary", exist_ok=True
        )
        ...
        base_path = output_path / "gr_sepsis_summary"
        ...
        pd.DataFrame(error_list, columns=["csn", "error"]).to_csv(
            base_path / "error_summary" / ("error_list_" + UNIQUE_FILE_ID + ".csv"),
            index=False,
        )
        # write sepsis files
        pd.concat(appended_sofa_scores).to_csv(
            base_path / "sofa_summary" / ("sofa_summary_" + UNIQUE_FILE_ID + ".csv"),
            index=False,
        )
        pd.concat(appended_sep3_time).to_csv(
            base_path / "sep3_summary" / ("sepsis3_summary_" + UNIQUE_FILE_ID + ".csv"),
            index=False,
        )
        pd.concat(appended_sirs_scores).to_csv(
            base_path / "sirs_summary" / ("sirs_summary_" + UNIQUE_FILE_ID + ".csv"),
            index=False,
        )
        pd.concat(appended_sep2_time).to_csv(
            base_path / "sep2_summary" / ("sepsis2_summary_" + UNIQUE_FILE_ID + ".csv"),
            index=False,
        )
