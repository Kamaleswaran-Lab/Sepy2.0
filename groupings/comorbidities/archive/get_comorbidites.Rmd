...
```{r}
comorbid_map = icd10_map_ahrq
mx <- max(lengths(comorbid_map))
df <- data.frame(lapply(comorbid_map, `length<-`, mx))
ICD10_ahrq <- gather(df, ahrq, ICD10, CHF:Depression, na.rm = TRUE)

write.csv(ICD10_ahrq,"ICD10_ahrq.csv",row.names = FALSE)
...
```{r}
comorbid_map = icd10_map_elix
mx <- max(lengths(comorbid_map))
df <- data.frame(lapply(comorbid_map, `length<-`, mx))
ICD10_elix <- gather(df, elix, ICD10, CHF:Depression, na.rm = TRUE)
write.csv(ICD10_elix,"ICD10_elix.csv",row.names = FALSE)
...
```{r}
comorbid_map = icd10_map_quan_deyo
mx <- max(lengths(comorbid_map))
df <- data.frame(lapply(comorbid_map, `length<-`, mx))
ICD10_quan_deyo <- gather(df, quan_deyo, ICD10, MI:HIV, na.rm = TRUE)
write.csv(ICD10_quan_deyo,"ICD10_quan_deyo.csv",row.names = FALSE)
...
```{r}
comorbid_map = icd10_map_quan_elix
mx <- max(lengths(comorbid_map))
df <- data.frame(lapply(comorbid_map, `length<-`, mx))
ICD10_quan_elix <- gather(df, quan_elix, ICD10, CHF:Depression, na.rm = TRUE)
write.csv(ICD10_quan_elix,"ICD10_quan_elix.csv",row.names = FALSE)
...
## CCS
```{r}
ccs_labels <- read.table('ccs_labels.csv', 
                   header=TRUE,
                   sep=',')
...
comorbid_map = icd10_map_ccs$single
mx <- max(lengths(comorbid_map))
df <- data.frame(lapply(comorbid_map, `length<-`, mx))
ICD10_single_ccs <- gather(df, ccs_code, ICD10, X0:X2621, na.rm = TRUE)
ICD10_single_ccs$ccs_code<-gsub("X","", ICD10_single_ccs$ccs_code)
ICD10_single_ccs <- ICD10_single_ccs %>%
                  left_join(ccs_labels,by=c("ccs_code"="CCS.DIAGNOSIS.CATEGORIES")) %>%
                  rename(ccs_label = CCS.DIAGNOSIS.CATEGORIES.LABELS)
write.csv(ICD9_single_ccs,"ICD10_single_ccs.csv",row.names = FALSE)
